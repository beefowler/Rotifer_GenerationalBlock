...
figure
% Testing that benefit of generational delay does not depend on late clones
% only
...
bmax=2.1; q=0.5; K=500; c=0.2; H=0; tau = 1; 
...
%phenotypes
m_i_vals = [.5 .5] %[0.1 0.16]; 
T_i = 20; 


%Two different delay options
 Gvals = [0 6]; % 

for a = 1:2 %cycle through two delay options 
    G_i = Gvals(a); 
    m_i = m_i_vals(a)
    ...
    phi = R_y ./length(sk); % figure out how many resting eggs will emerge each hatch day
        ...
        %run simulation
        sol=dde23(@(t,x,x_hists) gen_rotifer6(t, x, x_hists,tau,bmax,q,K,c,G_i,m_i,T_i, phi, sk), tau, @history_fun, tspan);
...
%title(['m_i = ' num2str(m_i) ', T_i= ' num2str(T_i) ', G_i= ' num2str(Gvals(a)), ', q= ', num2str(q)])
title(['G_i= ' num2str(Gvals(a))])

hold on 
plot([t(1) t(end)], [T_i T_i], '--', 'Color', [.3 .3 .3])
box on 
h = legend({'Resting eggs'; 'Mictic females'; 'Amictic adults'; 'Population size'; 'Density threshhold';});
