...
num_seasons = 40;
a_num = length(mixis_and_G); %need this defined for parfor loop
for a=1:a_num %previously stopped at a = 25, b = 355
    parfor b = 1:a_num
        if a == b
            continue
        end
        
        pheno_1 = mixis_and_G(:,a);
        pheno_2 = mixis_and_G(:,b);

        invasion_prop = nan(1,num_experiments);
        total_pop = nan(1, num_experiments); 

        G_i = [pheno_1(2) pheno_2(2)]
        %choose generation delay for both phenotypes
        if G_i(1) ~= 0 && G_i(2) ~=0
        ...
        if exist(['Invasion_with_2_params/outputs_' num2str(pheno_1(1)) '_G' num2str(pheno_1(2)) 'by' num2str(pheno_2(1)) '_G' num2str(pheno_2(2)) '.mat'], 'file')
            continue
        end
        ...
        for x = 1:num_experiments
            ...
            for season = 1:num_seasons
                ...
                %run simulation
                sol=dde23(@(t,x,x_hists) gen_rotifer5(t, x, x_hists,tau,bmax,q,K,c,G_i,m_i,T_i, phi, sk), tau, @(t) history_fun(t, sk, x_width), tspan);
...
save(['Invasion_with_2_params/outputs_' num2str(pheno_1(1)) '_G' num2str(pheno_1(2)) 'by' num2str(pheno_2(1)) '_G' num2str(pheno_2(2)) '.mat'], 'pheno_1', 'pheno_2', 'invasion_prop', 'num_seasons', 'total_pop')
end
