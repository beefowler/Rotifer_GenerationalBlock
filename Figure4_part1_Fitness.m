...
% used to create the data for Figure 4 of the manuscript. 
...
mixis = [0:0.05:1]; %mixis values along x axis 
...
fitness_all = cell(a_num, num_experiments); 
...
for x = 1:num_experiments %for each of 40 independent runs
    ...
    for a=1:a_num %for each phenotype
       ...
       fitness_all{a, x} = season_fitness;

    end

end

Average_fitness = cellfun(@mean, fitness_all)

 save('Data_for_Figure4_Fitness.mat', 'mixis', 'G_vals', 'mixis_and_G', 'num_seasons', 'num_experiments', 'cumulative_egg_production', 'fitness_all')
...
%phenotypes to try 
mixis = [0:0.05:1]; %mixis values along x axis 
threshold = [0 .4 .8 1.6 3.2 6.4]; % values of T_i for each of the lines when Gi=0 
...
if skipG == 0
 save('Data_for_Figure4_Fitness.mat', 'mixis', 'G_vals', 'threshold', 'mixis_and_G', 'mixis_and_thresh', 'num_seasons', 'num_experiments', 'cumulative_egg_production', 'cumulative_egg_production_2', 'fitness_all_2', 'fitness_all', '-append')
else 
 save('Data_for_Figure4_Fitness.mat', 'mixis', 'G_vals', 'threshold', 'mixis_and_G', 'mixis_and_thresh', 'num_seasons', 'num_experiments', 'cumulative_egg_production', 'cumulative_egg_production_2', 'fitness_all_2', 'fitness_all')
end
