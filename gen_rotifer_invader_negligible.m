...
%Second phenotype is assumed to be invader at neglibile abundance !
...
function dxdt = gen_rotifer_invader_negligible(t, x, x_hist,tau,bmax,q,K,c,G_i,m_i,T_i, phi, sk)
    ...
    num_pops = 2;
        ...
        ind = find([length(G_i) length(m_i) length(T_i)] ~= num_pops);
        for i = ind
            if i == 1
    ...
    if any(x<0)
        %disp(x)
        x(x<0)=0;  % and make sure no negative values 
    end
    ...
    %Invader is negligible so total_pop_size is only based on resident
    total_pop_size = sum(X_all_1(2:end)); %sum all individuals (not eggs) of first strain 
    ...
    %some minor rounding errors happening but should never lose resting 
    %eggs
    if dxdt_1(1)<0
        dxdt_1(1) = 0; 
    end
    ...
    %Threshhold is based on total population size! Invader is negligible so total_pop_size is only based on resident
    total_pop_size_hist = sum(X_hist_all_1(2:end)); %sum all individuals (not eggs) of first strain 
